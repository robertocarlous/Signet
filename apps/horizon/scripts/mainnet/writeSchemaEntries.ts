...
/**
 * MAINNET Schema Attestation Processor
 *
 * This script processes each JSONL schema file, matches schema names to values
 * from the schema value definitions, creates attestations for those schemas,
 * and updates the database with category information.
 *
 * Uses existing wallet via SECRET_KEY argument for mainnet deployments.
 *
 * Usage: pnpm tsx scripts/mainnet/writeSchemaEntries.ts --secret=SXXXXXXX
 */

import { readFileSync, readdirSync } from 'fs';
import { join } from 'path';
import { Keypair, Transaction } from '@stellar/stellar-sdk';
import * as ProtocolContract from '@signetprotocol/stellar-contracts/protocol';
  ...
  try {
    ...
    const sent = await tx.signAndSend({
      signTransaction: async (xdr: string) => {
        const transaction = new Transaction(xdr, ProtocolContract.networks.mainnet.networkPassphrase);
 ...
 */
async function main() {
  console.log('🚀 Starting MAINNET Schema Attestation Processing...\n');

  // Get secret key from command line argument
  const args = process.argv.slice(2);
  const secretKeyArg = args.find(arg => arg.startsWith('--secret='));

  if (!secretKeyArg) {
    console.error('❌ Secret key is required for mainnet operations');
    console.log('💡 Usage: npx ts-node mainnet/writeSchemaEntries.ts --secret=SXXXXXXX');
    console.log('   Example: npx ts-node mainnet/writeSchemaEntries.ts --secret=SABCDEF...');
    process.exit(1);
  }

  const secretKey = secretKeyArg.split('=')[1];

  try {
    // Use existing keypair from secret key
    const keypair = Keypair.fromSecret(secretKey);
    console.log(`🔑 Using existing keypair: ${keypair.publicKey()}`);
    console.log(`⚠️  MAINNET MODE - Real XLM will be spent!`);

    // Create protocol client for mainnet
    const protocolClient = new ProtocolContract.Client({
      contractId: MAINNET_PROTOCOL_CONTRACT_ID,
      networkPassphrase: ProtocolContract.networks.mainnet.networkPassphrase,
      rpcUrl: MAINNET_RPC_URL,
      allowHttp: false,
      publicKey: keypair.publicKey()
    });

    // Find all JSONL schema files in mainnet directory
    const files = readdirSync(SCRIPTS_MAINNET_DIR)
      .filter(file => file.startsWith('schemas-') && file.endsWith('.jsonl'))
      .map(file => join(SCRIPTS_MAINNET_DIR, file));

    if (files.length === 0) {
      console.log('❌ No schema JSONL files found in mainnet scripts directory');
      return;
    }
    ...
    // Print summary
    console.log('\n' + '='.repeat(60));
    console.log('📊 MAINNET PROCESSING SUMMARY');
    ...
    console.error('❌ MAINNET schema attestation processing failed:', error);
    process.exit(1);
  }
}
