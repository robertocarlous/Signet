...
async function main() {
  console.log('🚀 Starting MAINNET schema registration process...\n');

  // Get category to process from command line argument
  const args = process.argv.slice(2);
  const secretKeyArg = args.find(arg => arg.startsWith('--secret='));
  const categoryToProcess = args.find(arg => !arg.startsWith('--'));

  if (!secretKeyArg) {
    console.error('❌ Secret key is required for mainnet operations');
    console.log('💡 Usage: npx ts-node mainnet/importSchema.ts <category> --secret=SXXXXXXX');
    console.log('   Example: npx ts-node mainnet/importSchema.ts identity --secret=SABCDEF...');
    process.exit(1);
  }
  ...
  if (!categoryToProcess) {
    ...
    console.log(`\n💡 Usage: npx ts-node mainnet/importSchema.ts <category> --secret=SXXXXXXX`);
    console.log(`   Example: npx ts-node mainnet/importSchema.ts identity --secret=SABCDEF...`);
    return;
  }
  ...
  try {
    ...
    // Get contract ID from environment or use mainnet deployment
    const protocolContractId = MAINNET_PROTOCOL_CONTRACT_ID;
    const rpcUrl = MAINNET_RPC_URL;

    console.log(`Using mainnet protocol contract: ${protocolContractId}`);
    ...
    // Save results to category-specific file in mainnet directory
    ...
    console.log(`\n✅ MAINNET category '${categoryToProcess}' processing complete!`);
    ...
    if (results.length > 0) {
      console.log(`\n🎉 Successfully registered schemas on MAINNET:`);
    ...
    console.error('❌ MAINNET schema registration failed:', error);
    process.exit(1);
  }
}
