...
// Register a schema - the walletClient's account becomes its authority
const { uid: schemaUID } = await signet.registerSchema({
  definition: 'string name,bool verified,uint64 timestamp',
  revocable: true,
  // resolver: undefined  // omit = no resolver = permissionless attestations
})
...
On Stellar testnet, `network: 'testnet'` resolves to the current protocol contract, <code>{testnetCurrent}</code>. Pass `contractId` only to pin an older version. On Monad, `chain: 'monadTestnet'` resolves to the addresses in `DEPLOYMENTS` (see [Monad overview](/monad/overview)).
...
const { uid: schemaUID } = await signet.registerSchema({
  ...
  resolver: '0xResolverContract...', // Resolver controls who can attest
  revocable: true,
})
...
## Verified Authority (Optional, Stellar only)

<Note>
Verified Authority registration is currently a Stellar-specific feature — there is
no separate on-chain "verified issuer" registry on Monad. On Monad, an attester's
trustworthiness is something your app decides off-chain (e.g. an allowlist in a
[resolver](/concepts/resolvers), or a list your app maintains) rather than a
protocol-level badge.
</Note>

Separately from schema authority, you can register as a **Verified Authority** on the SignetProtocol platform (Stellar). This is completely optional and provides:
...
Authorities can delegate attestation signing to other addresses — EIP-712 on Monad,
BLS on Stellar. This enables:
...
<CardGroup cols={2}>
  ...
  <Card title="Delegates" icon="signature" href="/concepts/delegates">
    Enable off-chain signing — EIP-712 on Monad, BLS on Stellar
  </Card>
