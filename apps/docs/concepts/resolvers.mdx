...
The hooks and semantics are identical across chains — only the language differs:

<CodeGroup>
```solidity Monad
interface IResolver {
    // Pre-creation gate. Return false (or revert) to block.
    function onAttest(Attestation calldata attestation) external returns (bool allowed);

    // Pre-revocation gate. Same semantics.
    function onRevoke(Attestation calldata attestation) external returns (bool allowed);

    // Post-processing, best-effort (failure doesn't undo the attestation).
    function onResolve(bytes32 attestationUID, address attester) external;

    // Descriptive metadata for discovery/integration.
...
<Tip>
On Monad you don't usually implement `IResolver` directly — extend the abstract
`SchemaResolver` base contract instead. It wires the hooks, restricts calls to the
attestation registry, and gives you simple `_onAttest` / `_onRevoke` overrides (see
[Creating a Custom Resolver](#creating-a-custom-resolver) below).
</Tip>
...
struct Attestation {
    bytes32 uid;
    bytes32 schemaUID;
    address subject;         // who is being attested about
    address attester;        // who is attesting
    uint64 nonce;
...
<Note>
The patterns below (Default, Fee Collection, Token Reward, Authority-gated) are
shown as Soroban/Rust pseudocode, but the same logic applies on Monad through the
same four hooks — see [Creating a Custom Resolver](#creating-a-custom-resolver)
for a real Solidity example (`SampleResolver`, an allowlist gate).
</Note>
...
<Tabs>
  <Tab title="Monad">
    ### Step 1: Extend `SchemaResolver`

    `SchemaResolver` is the abstract base every resolver in `contracts/evm` uses —
    it wires the `IResolver` hooks and restricts calls to the attestation
    registry, leaving you just `_onAttest` / `_onRevoke` / `_onResolve` /
    `metadata` to override. This is the real `SampleResolver` shipped in the
    repo — an allowlist gate:
    ...
    contract SampleResolver is SchemaResolver {
        ...
        constructor(address attestationRegistry_, address owner_)
            SchemaResolver(attestationRegistry_)
        {
            owner = owner_;
        }
        ...
        // Override just the hook you need — defaults accept everything.
    ...
    ```bash
    forge create src/resolvers/SampleResolver.sol:SampleResolver \
      --rpc-url monad_testnet --private-key 0x… \
      --constructor-args <attestationRegistryAddress> <ownerAddress> \
      --broadcast
    ```
    ...
    const { uid: schemaUID } = await signet.registerSchema({
      ...
      resolver: '0xYourDeployedResolver...',
      revocable: true,
    })
    ```
  </Tab>
...
const { uid: withResolver } = await signet.registerSchema({
...
const { uid: withoutResolver } = await signet.registerSchema({
  definition: 'string note',
  // resolver omitted = no resolver = anyone can attest
  revocable: false,
})
```
...
<CodeGroup>
```solidity Monad
// Monad resolvers just revert — no shared error enum required.
// The base SchemaResolver already reverts SignetErrors.NotAuthorized()
// for any caller that isn't the attestation registry.
if (!allowlisted[attestation.attester]) revert NotAllowed(attestation.attester);
```
...
<Tabs>
  <Tab title="Monad">
    1. **Restrict to the registry** — `SchemaResolver`'s `onlyRegistry` modifier already does this; don't bypass it.
    2. **Validate all inputs** — don't trust `attestation.data` blindly; decode and check it.
    3. **Revert with a clear reason** — custom errors (`revert MyError(...)`) beat silent `false` returns for debuggability.
    4. **Avoid state dependencies** — don't rely on external state (oracle prices, other contracts) that can be manipulated mid-transaction.
    5. **Test edge cases** — `forge test` expired attestations, revocations, self-attestation, etc.
  </Tab>
  <Tab title="Stellar">
...
|---------|-------|----------|
| Missing access check | Anyone can attest | Monad: keep `onlyRegistry`; Stellar: always call `require_auth()` |
| Self-attestation | Users attest to themselves | Check `attester != subject` |
| Expired data | Using outdated timestamps | Validate against current block/ledger time |
...
<CodeGroup>
```solidity Monad
// contracts/evm/test — run with `forge test`
function test_RejectsNonAllowlistedAttester() public {
    vm.prank(address(attestationRegistry));
    bool allowed = resolver.onAttest(buildAttestation(attacker, subject));
    assertFalse(allowed);
}

function test_AcceptsAllowlistedAttester() public {
    resolver.setAllowlisted(attester, true);
    vm.prank(address(attestationRegistry));
    bool allowed = resolver.onAttest(buildAttestation(attester, subject));
    assertTrue(allowed);
}
```
...
<CodeGroup>
```text Monad
SchemaResolver     abstract base — extend this for your own resolver
SampleResolver     reference allowlist gate (see above)
PersonhoodResolver locks the personhood schema to PasskeyAttester only
```

```bash Monad
cd contracts/evm
forge build
forge test   # 35 tests, including the resolvers
```
...
<CardGroup cols={2}>
  <Card title="Monad Overview" icon="rocket" href="/monad/overview">
    Live resolver addresses and contract architecture
  </Card>
