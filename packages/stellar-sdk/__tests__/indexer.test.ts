 ...
 * 
 * This test suite makes actual HTTP requests to signet-horizon-testnet.vercel.app
 * to verify all API fetch operations work correctly without mocking.
 * 
 * NOTE: Requires the local horizon server to be running at http://signet-horizon-testnet.vercel.app
 */
...
// Override the registry endpoint to use signet-horizon-testnet.vercel.app
REGISTRY_ENDPOINTS.testnet = 'http://signet-horizon-testnet.vercel.app/api/registry'
...
describe('Real API Fetch Endpoints Test Suite', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      const response = await fetch('http://signet-horizon-testnet.vercel.app/api/health')
      if (!response.ok) {
        console.warn('Warning: Local horizon server may not be running at http://signet-horizon-testnet.vercel.app')
      }
    } catch (error) {
      console.error('Error: Cannot connect to http://signet-horizon-testnet.vercel.app. Please ensure the local horizon server is running.')
    }
  })
