...
SignetSDK is a JavaScript library for creating and managing attestations across multiple blockchains, currently supporting Solana and Stellar networks.

## Installation

```bash
npm install @signetprotocol/sdk
```
...
```ts
import { SignetSDK } from '@signetprotocol/sdk'
...
async function run() {
  ...
  const client = await SignetSDK.initializeSolana({
...
async function run() {
  ...
  const client = await SignetSDK.initializeStellar({
